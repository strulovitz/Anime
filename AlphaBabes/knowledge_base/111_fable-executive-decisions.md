...
## The map (so you always know where we are)
...
## STEP 1 — Laptop: save the directive
Save as KB 110_MASTER-DIRECTIVE-both-machines.md

## STEP 2 — Laptop: scorched-earth mission
- Phase 1: Read-only inventory (du, conda env list, docker check, cache sizes)
- Phase 2: Delete (miniforge3, venvs, old projects, caches, docker)
- Phase 3: Rebuild foundation (fresh Miniforge3, verify)
- Then: immediately re-run ComfyUI mission from KB 090

## STEP 3 — Desktop: bootstrap
- Install OpenCode via curl
- auth login with OpenRouter API key
- Clone repo, run scorched-earth mission (same as Step 2, adapted for desktop)

## Today's scoreboard
- ⬜ KB save (Step 1) — 1 minute
