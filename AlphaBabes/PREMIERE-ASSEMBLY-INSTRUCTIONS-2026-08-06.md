...
- **YES, USE CHROMA KEY (green screen removal)** on every small talking-head clip. The Madie emotion images used to generate these Hedra/Kling clips were rendered on a GREEN SCREEN background (confirmed: `AlphaBabes/images/madie-emotions-green/` and a pre-flipped version in `AlphaBabes/images/madie-emotions-green-flipped/` both exist in the repo), so every clip in `hedra-clips/` should have a green background behind Madie that needs to be keyed out so only her animated character shows over the scene illustration.
  - In Premiere, apply the **Ultra Key** effect (Effects panel → search "Ultra Key") to each small clip on the overlay track.
  - Set the Key Color eyedropper to sample the green background directly from the clip.
  - Check the "Output" dropdown temporarily to "Alpha Channel" to verify the key is clean (Madie should be solid white, background solid black, with no holes in her hair/edges). Switch back to "Composite" when done checking.
  - Adjust Matte Generation (Pedestal) and Matte Cleanup (Choke/Soften) sliders as needed if there's green spill/fringing on her edges, especially around hair.
  - If a single Ultra Key doesn't fully clean up edges, a small amount of "Spill Suppression" or a subtle Garbage Matte (mask) can help, but don't over-engineer this — Ultra Key with reasonable defaults usually works well on clean green screen footage.
  - Once keyed, only Madie's character should be visible, floating over the full-screen scene illustration underneath — no green, no black box, just her overlaid directly on the scene.
  - **This replaces the earlier "keep it as a black comm-screen window" idea from an older session — that plan is OUTDATED. Green screen removal via Ultra Key is the correct, current approach. If you see any old notes elsewhere in this repo mentioning "don't chroma key" or "keep the black background," ignore them — this instructions file is the current, correct source of truth as of 2026-08-06.**
...
## 9. WHAT NOT TO DO
- Do NOT skip the chroma key step — every small talking-head clip MUST have its green background keyed out with Ultra Key (see section 1 above). Leaving the green in looks unfinished and wrong.
